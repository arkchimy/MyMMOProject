...
void fnAcceptExIOCPNetworkLib()
{
	...
	{
		virtual void onAccept(ull& sessionID) {};
...
namespace network
{
	...
	void NetworkLib::start()
	{
		for (int idx = 0; idx < CONFIG_WORKER_THREAD_CNT; ++idx)
		{
			...
			SetThreadDescription(mWorkerThreads[idx].native_handle(), threadName.c_str());
		}
	}
	...
	void NetworkLib::disconnectAllSession()
	{
		for (auto& Session : mSessions)
		{
			disconnectSession(Session->mSessionID,true);
		}
		while (mActiveSlotCnt != 0)
		{
			YieldProcessor();
		}
		//종료 메세지
	...
	void NetworkLib::registerAcceptEx()
	{
		...
		RT_ASSERT(session.mSock != INVALID_SOCKET);
		...
		*/

		{
			...
			// IO_PENDING(정상 비동기 진행) 또는 즉시완료 — 둘 다 결국 completion이 온다.
			// mSessionCnt(실제 접속자 수)가 아니라 mOutstandingSlotCnt(정리 대기용)를 올림 — 아직 진짜 접속은 아니므로.
			InterlockedExchange16(&session.mIOcnt, 1);
			_interlockedincrement64(&mActiveSlotCnt);
		}
	}
	...
	void NetworkLib::registerRecv(Session& session)
	{
		...
		char* f = session.mRecvBuffer->GetFrontPtr();
		char* r = session.mRecvBuffer->GetRearPtr();

		int32_t freeSize = session.mRecvBuffer->GetFreeSize(f, r);
		...
		wsabuf[0].buf = r;
		wsabuf[0].len = session.mRecvBuffer->GetDirectFreeSize(f, r);
	...
	void NetworkLib::completeRecv(Session& session, DWORD transferred)
	{
		...
		utility::MyRingBuffer& recvBuffer = *session.mRecvBuffer;
		recvBuffer.MoveRear(transferred);
		char* f = session.mRecvBuffer->GetFrontPtr();
		char* r = session.mRecvBuffer->GetRearPtr();

		int32_t useSize = recvBuffer.GetUseSize(f, r);

		while (sizeof(Header) < useSize)
		{
			f = session.mRecvBuffer->GetFrontPtr();
			r = session.mRecvBuffer->GetRearPtr();
			...
			if (header.Len + sizeof(Header) <= useSize)
			{
				...
				int32_t deqSize = recvBuffer.GetDirectUseSize(f, r);
			...
			f = session.mRecvBuffer->GetFrontPtr();
			r = session.mRecvBuffer->GetRearPtr();
			useSize = recvBuffer.GetUseSize(f, r);
		}
		registerRecv(session);
	}
	...
	void NetworkLib::registerSend(Session& session)
	{
		...
		session.mSendOv->mMsgCnt = msgCnt;
		...
		for (__int16 cnt = 0; cnt < msgCnt; ++cnt)
		{
			...
			session.mSendOv->mSendMsgs[cnt] = msg;
	...
	{
		// 진짜 accept 성공했던 세션만 mSessionCnt/onRelease 대상 (phantom accept는 제외)
		if (session.mAccepted)
		{
			onRelease(session.mSessionID);
			int64_t sessionCnt = _InterlockedDecrement64(&mSessionCnt);
			MY_ASSERT(0 <= sessionCnt, "음수는 말이안 됨");
		}
		_InterlockedDecrement64(&mActiveSlotCnt);
	...
	Session* const NetworkLib::FindSessionOrNull(const SeqAndIdx& seqId)
	{
		if (CONFIG_SESSION_MAX < seqId.Idx)
		{
			return nullptr;
		}
		RT_ASSERT(seqId.Idx < CONFIG_SESSION_MAX);
		if (mSessions[seqId.Idx]->mSessionID != seqId)
		{
			return nullptr;
		}
		return mSessions[seqId.Idx].get();
	}
	...
	void NetworkLib::disconnectSession(const SeqAndIdx& sessionID, bool isNetworklib)
	{
		if (sessionLock(sessionID) == false)
		{
			return;
		...
		CancelIoEx((HANDLE)session.mSock, session.mSendOv);
		CancelIoEx((HANDLE)session.mSock, session.mRecvOv);
		CancelIoEx((HANDLE)session.mSock, session.mAcceptOv);

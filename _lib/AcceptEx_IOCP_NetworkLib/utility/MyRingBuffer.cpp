...
do{\
	...
	{\
		...
		if(mBFail & (1 << static_cast<uint8_t>(eRingBufferFailBit::BufferPeekEmpty)))\
		{\
			std::cout << std::setw(10) << "BufferPeekEmpty\n";\
		}\
...
namespace utility
{
	...
	void MyRingBuffer::MoveRear(int32_t move)
	{
		...
		_interlockedexchange64((volatile long long*)&mRear,(volatile long long)r);
	}

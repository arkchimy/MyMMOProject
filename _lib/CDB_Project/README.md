...
snprintf(query, sizeof(query),
    "SELECT x, y FROM Characters WHERE account_no = %lld", accountNo);
...
`false`만 리턴한다. 이걸 보고도 `ClearError()` 안 하고 **다음 CDB 함수를 또 호출하면**,
그 함수 진입하자마자 이전 실패로 `CDBException`이 던져진다. 그래서 CDB를 쓰는 코드는
try-catch로 감싸는 게 안전하다:

```cpp
try
{
    db.Execute(query1);   // 실패해도 여기선 조용함 (bFailed만 세팅됨)
    db.Execute(query2);   // ClearError() 안 했으므로 진입하자마자 예외 던짐
}

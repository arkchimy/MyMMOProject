...
void CProfileManager::CreateProfile(const wchar_t* fileName)
{
	...
	{
		ZeroMemory(buffer, sizeof(buffer));
		if (mEntrys[i].mTotalCnt > (int64_t)eConfig::AbnormalBufferSize * 2)
		{
			int64_t totalTime = mEntrys[i].mTotalTime;
			int64_t abnormalSum = 0;
			for (int64_t j = 0; j < (int64_t)eConfig::AbnormalBufferSize; j++)
			{
			...
			mEntrys[i].mAvg = (static_cast<double>(totalTime) / (mEntrys[i].mTotalCnt - (int64_t)eConfig::AbnormalBufferSize * 2)) * 1e6 / mQpcFrequency.QuadPart;

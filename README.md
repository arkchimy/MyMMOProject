...
랜덤 인카운터 대신 **여러 유저가 한 필드에서 동시에 몬스터와 실시간 전투를 벌이는 구조**를 택했습니다. 이 한 장면 안에 IOCP 비동기 I/O, 스레드 간 무락(lock-free) 통신, 몬스터를 잡을 때마다 MySQL과 Redis 랭킹이 동시에 갱신되는 구조까지 서버 개발의 핵심 기술이 담겨 있습니다.
...
> **설계 배경, 트러블슈팅 과정, 코드 상세 설명**은 [PORTFOLIO.md](./PORTFOLIO.md)에서 확인하실 수 있습니다.
...
[클라이언트] ── TCP
        ...
        │  ├─ authThread ── MySQL            │
        │  └─ fieldThread × N                │
        │       └─ dbThread × N ── MySQL/Redis│
        │           (필드당 1:1, 각자 전용 커넥션) │
        └─────────────────────────────────────┘
```

- 인증 · 필드 전투 · DB 저장을 모두 FieldServer 하나가 처리합니다. 스케일아웃 전까지는 별도 LoginServer가 불필요하다고 판단했습니다 (판단 근거는 [PORTFOLIO.md](./PORTFOLIO.md) 참고).
...
각 항목의 코드와 상세 설명은 [PORTFOLIO.md](./PORTFOLIO.md)에 있습니다.
...
1. MySQL에 DB를 생성하고 `Accounts` 테이블을 만듭니다 (코드에서 쓰는 컬럼 기준):
   ...
   | id | VARCHAR(19), UNIQUE | 로그인 ID (닉네임으로도 재사용) |
   | pw | VARCHAR(64) | 비밀번호 (평문 저장, SQL 인젝션 방지 이스케이프만 적용) |
   | x, y | FLOAT | 마지막 저장 위치 |
   | killCount | BIGINT, DEFAULT 0 | 몬스터 처치 수 (Redis ZSET과 이중 갱신) |
...
더 깊은 설계 의도와 트러블슈팅 과정이 궁금하시다면 [PORTFOLIO.md](./PORTFOLIO.md)를 참고해 주세요.
